...
AT.prototype.atPwdFormEvents = {
    ...
    "submit #at-pwd-form": function(event, t) {
        ...
        if (state === "signIn") {
            ...
            return Meteor.loginWithPassword(loginSelector, password, function(error) {
                ...
                if(!error) {
                  ...
                  var decryptedUserKey = CryptoJS.AES.decrypt(user.profile.privateKey, password).toString(CryptoJS.enc.Utf8);

                  Session.setAuth('privateKey', decryptedUserKey);
                }
            });
        }
        ...
        if (state === "signUp") {
            ...
            var profileHooks = Accounts.extendProfile(password);
            _.extend(formData, profileHooks.profileExtension);
